...
file_decompressor(struct file_compression_config *app_cfg, struct program_core_objects *state, uint64_t original_checksum)
{
	...
	// TODO: move below checksum check
	...
	// TODO: write_local_file should be here
...
int main() {
	...
	strlcpy(app_cfg.file_path, "/dev/shm/input", MAX_FILE_NAME);
	strlcpy(app_cfg.output_file_path, "/dev/shm/input-comp.txt", MAX_FILE_NAME);
	app_cfg.compress_method = COMPRESS_DEFLATE_HW;
	...
	strlcpy(app_cfg.file_path, "/dev/shm/input-comp.txt", MAX_FILE_NAME);
	strlcpy(app_cfg.output_file_path, "/dev/shm/input-decomp", MAX_FILE_NAME);
	app_cfg.compress_method = COMPRESS_DEFLATE_HW;
