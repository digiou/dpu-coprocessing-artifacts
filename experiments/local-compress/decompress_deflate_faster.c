...
DOCA_LOG_REGISTER(DECOMPRESS_DEFLATE_FASTER::MAIN);
...
int offload_next(struct compression_state *state) {
    ...
    struct doca_compress_task_decompress_deflate *compress_task;
    err = doca_compress_task_decompress_deflate_alloc_init(state->compress, buf_in, buf_out, task_user_data, &compress_task);
    if(err != DOCA_SUCCESS) {
        DOCA_LOG_ERR("Failed to allocate task %zu: %s", num, doca_error_get_descr(err));
        goto failure_buf_out;
    }

    err = doca_task_submit(doca_compress_task_decompress_deflate_as_task(compress_task));
    if(err != DOCA_SUCCESS) {
...
void chunked_decompress_error_callback(struct doca_compress_task_decompress_deflate *compress_task,
                                     union doca_data task_user_data, union doca_data ctx_user_data) {
    doca_error_t err = doca_task_get_status(doca_compress_task_decompress_deflate_as_task(compress_task));
    size_t num = (size_t) task_user_data.u64;
    DOCA_LOG_ERR("Task %zu failed: %s", num, doca_error_get_descr(err));

    struct doca_buf const *buf_in = doca_compress_task_decompress_deflate_get_src(compress_task);
    struct doca_buf *buf_out = doca_compress_task_decompress_deflate_get_dst(compress_task);
...
void chunked_decompress_completed_callback(struct doca_compress_task_decompress_deflate *compress_task,
                                         union doca_data task_user_data, union doca_data ctx_user_data) {
    size_t num = (size_t) task_user_data.u64;
    struct compression_state *state = (struct compression_state *) ctx_user_data.ptr;
...
struct doca_compress *open_compress_context(struct doca_dev *dev, struct doca_pe *engine,
    ...
    err = doca_compress_task_decompress_deflate_set_conf(compress, chunked_decompress_completed_callback, chunked_decompress_error_callback, state->num_buffers);
    if(err != DOCA_SUCCESS) {
...
int main(int argc, char **argv) {
    ...
    FILE *ofp = NULL;
    doca_error_t result;
	char file_path[256] = "/dev/shm/input-comp.deflate";
	char output_path[256] = "/dev/shm/output-decomp.deflate";
	...
	DOCA_LOG_INFO("In decompress_file. file size %ld, job type DOCA_DECOMPRESS_DEFLATE_JOB, decompress_method DEFLATE", file_size);
