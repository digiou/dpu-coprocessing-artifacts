...
import argparse, os, pathlib, shutil, subprocess, random, zlib
from datetime import datetime

def parse_args():
    parser = argparse.ArgumentParser(description="DOCA DEFLATE throughput experiments.")
...
def make_dflt_fs(num_files: int=1024, block_size: int=64*1024, 
                root_dir: pathlib.Path=pathlib.Path("/dev/shm")) :
    cmp_dir = root_dir / "deflate_raw"
    shutil.rmtree(path=cmp_dir, ignore_errors=True)
    cmp_dir.mkdir(parents=True, exist_ok=True)
    fs_path = root_dir / "deflate_raw_stream.fs"
    fs_path.unlink(missing_ok=True)

    for i in range(num_files):
        plain = os.urandom(block_size)
        # Each call is one independent compressed block
        # Older zlib: use a compressobj to control wbits.
        # Positional args are most compatible: level=1, method=DEFLATED, wbits=-15 (raw)
        cobj = zlib.compressobj(1, zlib.DEFLATED, -15)
        comp = cobj.compress(plain) + cobj.flush()
        (cmp_dir / f"{i:04}.deflate").write_bytes(comp)

    with fs_path.open("w") as fs:
        for f in sorted(cmp_dir.glob("*.deflate")):
            fs.write(str(f) + "\n")
...
def execute_experiment(device_address: str, device_str: str, on_dpu: bool):
    ...
    if device_str == "bf2":
        ...
        # 2MiB (2_097_152) of input can expand to larger sizes due to deflate
    ...
    csv_output_path = f"/tmp/dflt-results-{device_str}-{timestamp}.csv"

    for job_size in job_sizes:
        make_dflt_fs(block_size=job_size)
        for threads in thread_counts:
            for batch in batch_sizes:
                cmd = [
                    ...
                    "--data-provider-input-file", "/dev/shm/deflate_raw_stream.fs",
