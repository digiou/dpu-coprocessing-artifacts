...
DOCA_LOG_REGISTER(DECOMPRESS_LZ4_BLOCK_FASTER::MAIN);
...
int offload_next(struct compression_state *state) {
    ...
    size_t offset = state->input_buffer_size * num;
    size_t output_offset = state->output_buffer_size * num;

    struct doca_buf *buf_in;
    struct doca_buf *buf_out;

    err = doca_buf_inventory_buf_get_by_data(state->buf_inv, state->mmap_in, state->in + offset, state->input_buffer_size, &buf_in);
    if(err != DOCA_SUCCESS) {
        DOCA_LOG_ERR("Failed to get inventory input buffer: %s for task: %zu", doca_error_get_descr(err), num);
        goto failure;
    }

    err = doca_buf_inventory_buf_get_by_addr(state->buf_inv, state->mmap_out, state->out + output_offset, state->output_buffer_size, &buf_out);
    if(err != DOCA_SUCCESS) {
    ...
    err = doca_compress_task_decompress_lz4_block_alloc_init(state->compress, buf_in, buf_out, task_user_data, &compress_task);
    if(err != DOCA_SUCCESS) {
        DOCA_LOG_ERR("Failed to allocate task %zu: %s", num, doca_error_get_descr(err));
        goto failure_buf_out;
    }

    err = doca_task_submit(doca_compress_task_decompress_lz4_block_as_task(compress_task));
    if(err != DOCA_SUCCESS) {
...
failure_task:
    doca_task_free(doca_compress_task_decompress_lz4_block_as_task(compress_task));
failure_buf_out:
...
void decompress_state_changed_callback(union doca_data user_data, struct doca_ctx *ctx, 
...
void chunked_decompress_error_callback(struct doca_compress_task_decompress_lz4_block *compress_task,
                                     union doca_data task_user_data, union doca_data ctx_user_data) {
    doca_error_t err = doca_task_get_status(doca_compress_task_decompress_lz4_block_as_task(compress_task));
    size_t num = (size_t) task_user_data.u64;
    DOCA_LOG_ERR("Task %zu failed: %s", num, doca_error_get_descr(err));

    struct doca_buf const *buf_in = doca_compress_task_decompress_lz4_block_get_src(compress_task);
    struct doca_buf *buf_out = doca_compress_task_decompress_lz4_block_get_dst(compress_task);
...
void chunked_decompress_completed_callback(struct doca_compress_task_decompress_lz4_block *compress_task,
                                         union doca_data task_user_data, union doca_data ctx_user_data) {
    size_t num = (size_t) task_user_data.u64;
    struct compression_state *state = (struct compression_state *) ctx_user_data.ptr;
    ...
    if(state->offloaded < state->num_buffers) {
        ...
        doca_ctx_stop(doca_compress_as_ctx(state->compress));
        clock_gettime(CLOCK_MONOTONIC, &state->post_ctx_end);
    }
}
...
struct doca_compress *open_compress_context(struct doca_dev *dev, struct doca_pe *engine,
    ...
    err = doca_ctx_set_state_changed_cb(ctx, decompress_state_changed_callback);
    if(err != DOCA_SUCCESS) {
    ...
    err = doca_compress_task_decompress_lz4_block_set_conf(compress, chunked_decompress_completed_callback, chunked_decompress_error_callback, state->num_buffers);
    if(err != DOCA_SUCCESS) {
...
struct doca_dev *open_compress_device(void) {
    ...
    DOCA_LOG_ERR("No LZ4 block compression device found");
...
struct region *decompress_buffers(void *indata, void *outdata, struct region *out_region, 
                                size_t num_buffers, size_t input_buffer_size, size_t output_buffer_size) {
    ...
    struct doca_mmap *mmap_in = open_memory_map(indata, num_buffers * input_buffer_size, compress_dev, DOCA_ACCESS_FLAG_LOCAL_READ_WRITE);
    if(!mmap_in) {
        goto cleanup_dev;
    }

    struct doca_mmap *mmap_out = open_memory_map(outdata, num_buffers * output_buffer_size, compress_dev, DOCA_ACCESS_FLAG_LOCAL_READ_WRITE);
    if(!mmap_out) {
    ...
    DOCA_LOG_INFO("Offloaded tasks: %" PRIu64 ", completed: %" PRIu64, state.offloaded, state.completed);
    ...
    if (task_exec_time < 0) {
        ...
        double data_rate_from_sec = num_buffers * input_buffer_size / task_exec_time / 1048576.0;
        DOCA_LOG_INFO("Callback task time: %.9f seconds", task_exec_time);
        double ctx_teardown = timespec_diff_sec(&state.post_ctx_end, &state.end);
        DOCA_LOG_INFO("Ctx teardown time: %.9f seconds", ctx_teardown);
        DOCA_LOG_INFO("Callback task throughput: %.9f mbps", data_rate_from_sec);
        double data_rate_from_task_only = num_buffers * input_buffer_size / task_only_execution_time / 1048576.0;
...
doca_error_t decompress_file(FILE *in, FILE *out, const uint32_t num_buffers, size_t input_buffer_size, size_t output_buffer_size) {
    DOCA_LOG_INFO("Input buffer number: %d", num_buffers);
    DOCA_LOG_INFO("Input buffer size: %ld", input_buffer_size);
    DOCA_LOG_INFO("Allocated dst buffer number: %d", num_buffers);
    DOCA_LOG_INFO("Allocated dst buffer size: %ld", output_buffer_size);

	// Allocate aligned memory using posix_memalign.
    // Note: The alignment must be a power of two and a multiple of sizeof(void *).
    uint8_t *indata = NULL;
    uint8_t *outdata = NULL;
    int ret = posix_memalign((void **)&indata, 64, num_buffers * input_buffer_size);
    if (ret != 0) {
        DOCA_LOG_ERR("posix_memalign failed for indata: %d", ret);
        return DOCA_ERROR_IO_FAILED;
    }
    ret = posix_memalign((void **)&outdata, 64, num_buffers * output_buffer_size);
    if (ret != 0) {
    ...
    size_t read_count = fread(indata, input_buffer_size, num_buffers, in);
    if (read_count != num_buffers) {
		DOCA_LOG_ERR("Failed fread on input file; expected %d buffers, got %zu", num_buffers, read_count);
        goto cleanup;
    }

    struct region *out_regions = decompress_buffers(indata, outdata, region_buffer, num_buffers, input_buffer_size, output_buffer_size);
    if(!out_regions) {
...
int main(int argc, char **argv) {
	if(argc < 4) {
        fprintf(stderr, "Usage: %s DEVICE BUFFER_SIZE NUM_COMPRESSED_BUFFERS ORIGINAL_FILE_SIZE\n", argv[0]);
        return EXIT_FAILURE;
    }
    ...
    size_t output_buffer_size = max_buffer_size;
    ...
    size_t num_compressed_buffers = strtol(argv[3], NULL, 10);

    size_t original_file_size = strtol(argv[4], NULL, 10);
    if (original_file_size <= max_buffer_size) { // output can fit whole in 1 output buffer
        output_buffer_size = original_file_size;
        num_compressed_buffers = 1;
    }
    ...
    FILE *ofp = NULL;
    doca_error_t result;
	char file_path[256] = "/dev/shm/input-comp.lz4";
	char output_path[256] = "/dev/shm/output-decomp.lz4";
    ...
    if (num_compressed_buffers == 1) {
        input_buff_size = file_size; // whole input and output fit in their respective buffer sizes
    }

    DOCA_LOG_INFO("Starting decompression");
	DOCA_LOG_INFO("In decompress_file. file size %ld, job type DOCA_DECOMPRESS_LZ4_BLOCK_JOB, decompress_method LZ4 BLOCK", file_size);
	...
	decompress_file(ifp, ofp, num_compressed_buffers, input_buff_size, output_buffer_size);
