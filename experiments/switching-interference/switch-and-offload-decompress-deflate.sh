...
SERVER_LOG_TPUT="${RAMLOG_DIR}/iperf_srv_decomp_dflt_throughput_${TIMESTAMP}.json"
SERVER_LOG_LATENCY="${RAMLOG_DIR}/iperf_srv_decomp_dflt_latency_${TIMESTAMP}.json"
...
echo "[`date`]  starting iperf3 server on DPU-1 for dflt"
...
ssh -n -f -o BatchMode=yes -o StrictHostKeyChecking=accept-new \
    ...
    "nohup /dev/shm/${HOST_EXECUTOR_LOCATION} --deflate > /dev/null 2>&1 &"
